let http = require('http');//на след итер ц событ
let url = require("url");
let fs = require('fs');
...
async function factCycleTick(req, res) {
    ...
    for (let k = 1; k < 1000; k++) {
        process.nextTick(async () => {//следующий шаг
            result += (n++) + '.Результат: ' + (Date.now() - d) + '-' + k + '/' + getSimpleFact(k) + '<br/>';//получить простой результат
            await sleep(1);
        });
    }
    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
    setTimeout(() =>//вызвать один раз через инт времени 
        res.end(result), 100);
}
