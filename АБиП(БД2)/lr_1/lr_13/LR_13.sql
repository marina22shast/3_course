...
--1.должна вывод список преподавателей, работающих на кафедре заданной кодом в параметре.

create or replace procedure GET_TEACHERS (PCODE TEACHER.PULPIT%TYPE) is
  cursor curs1 is select TEACHER_NAME, TEACHER
    from TEACHER
    where PULPIT = PCODE;--кафедра=подать заявку
...
begin
  ...
  loop
    ...
    exit when curs1%notfound;--не найдено
  end loop;
  close curs1;
end;
--выражает значение через список параметров
--drop procedure GET_TEACHERS;
-- анонимный блок продемонстрируйте выполнение процедуры.
begin
    GET_TEACHERS('ИСиТ');
end;

--2-3.количество преподавателей, работающих на кафедре заданной кодом в параметре
create or replace function GET_NUM_TEACHERS(PCODE TEACHER.PULPIT%TYPE)
  return number is--возвращаемый номер
    tCount number;
begin
  select count(*) into tCount--возвращает все строки
    from TEACHER
     where PULPIT = PCODE;
  return tCount;--выполняет 0 или более выражений
end;
...
--4.cписок преподователей на заданном кодом параметроа факультете
...
--5.количество преподавателей, работающих на факультете, заданным кодом в параметре
...
--продемонстрируйте выполнение процедур и функций
begin
