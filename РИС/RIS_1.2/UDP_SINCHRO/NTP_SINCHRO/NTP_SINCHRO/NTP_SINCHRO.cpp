...
int main() {
    ...
    if (!WSAStartup(MAKEWORD(2, 2), &wsaData)) {
        int times[] = { 1000,3000,6000,8000,10000,12000,14000 };//тикии из метолды//время между запросами
        for (int i = 0; i < 7; i++) {
            double avg = 0;
            double count = 0;
...
unsigned long long ntpdate() {
    ...
    // Хост ntp-server
    char* hostname = (char*)"vega.cbk.poznan.pl";//к нему подкл,это удал сервер
